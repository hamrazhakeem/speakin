...
import { ChevronDown, ChevronUp, Search, MessageCircle, Book, Settings, Users } from 'lucide-react';
  ...
  const FAQItem = ({ question, answer, isActive, onClick }) => (
    ...
    >
      <button
        ...
        onClick={onClick}
      >
        <span className={`text-lg font-medium ${isActive ? 'text-blue-600' : 'text-gray-900'} group-hover:text-blue-600 transition-colors`}>
          {question}
        </span>
        <div className={`ml-4 flex-shrink-0 p-2 rounded-full ${isActive ? 'bg-blue-100' : 'bg-gray-100'} group-hover:bg-blue-100 transition-colors`}>
          {isActive ? (
            <ChevronUp className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-gray-500'}`} />
          ) : (
