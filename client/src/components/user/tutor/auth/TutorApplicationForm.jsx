...
const TutorApplicationForm = () => {
		...
		const fetchData = async () => {
			try {
				...
				const countriesResponse = await fetch(
					`${import.meta.env.VITE_API_GATEWAY_URL}countries/`
				);
				...
				const languagesToTeachResponse = await fetch(
					`${import.meta.env.VITE_API_GATEWAY_URL}platform-languages/`
				);
				...
				const spokenLanguagesResponse = await fetch(
					`${import.meta.env.VITE_API_GATEWAY_URL}spoken-languages/`
				);
	...
	const handleAddSpokenLanguage = () => {
		setValue("spokenLanguages", [
			...watch("spokenLanguages"),
	...
	useEffect(() => {
		...
		if (loading) {
			toastId = toast.loading(
				"Please wait while we upload your introduction video and process your application..."
			);
		}
		return () => {
			if (toastId) {
	...
	const onSubmit = async (data) => {
		...
		try {
			const response = await axios.post(
				`${import.meta.env.VITE_API_GATEWAY_URL}tutor-request/`,
				formData,
				{
					headers: {
			...
			if (response.status === 201) {
				navigate("/tutor/request/application-confirmation", {
				...
				<div className="w-full max-w-3xl bg-white shadow-lg rounded-xl p-6 md:p-10 mt-16 mb-16">
					<div className="mb-10 text-center">
						...
						<p className="text-gray-600 text-base md:text-lg">
							Join our community of expert language tutors
						</p>
					</div>
					...
					<form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
								...
								rules={{
									...
									validate: {
										...
										noStartingOrEndingSpaces: (value) =>
											value.trim() === value ||
											"Name must not start or end with spaces",
										lowercase: (value) =>
								...
								rules={{
									...
									validate: {
										...
										noMultipleSpaces: (value) =>
											!/\s{2,}/.test(value) ||
											"Full Name must not contain multiple spaces between names",
										noStartingOrEndingSpaces: (value) =>
											value.trim() === value ||
						...
						<div className="space-y-4">
							...
							<Controller
								...
								render={({ field }) => (
									<SelectField
										{...field}
										options={languagesToTeach.map((lang) => ({
											value: lang.name,
											label: lang.name,
										}))}
							...
							<Controller
								...
								render={({ field: { onChange, value } }) => (
									<FileUpload
										...
										maxSize={5}
										label={`Upload your ${
											isNative ? "Government ID" : "Teaching Certificate"
										}`}
										error={errors.image}
									/>
								)}
						...
						<div className="space-y-2">
							...
							<p className="text-sm text-gray-600">
								Set hourly rate in Credits, 1 Credit worth ₹150
							</p>
							<Controller
								name="hourlyRate"
								control={control}
