...
const TutorDashboardContent = () => {
	...
	async function fetchUserData() {
		try {
			const [tutorResponse, countriesResponse, languagesResponse] =
				await Promise.all([
			...
			setValue(
				"requiredCredits",
				tutorResponse.tutor_details?.required_credits || ""
			);
			...
			sessionStorage.setItem(
				"teachingLanguage",
				tutorResponse.tutor_language_to_teach[0].language
			);
			setCountries(countriesResponse);

			const languagesWithIds = languagesResponse.languages.map(
				(lang, index) => ({
					id:
						typeof lang === "string"
							? `lang-${index}`
			...
			const proficienciesWithIds = languagesResponse.proficiencies.map(
				(prof, index) => ({
					id:
						typeof prof === "string"
							? `prof-${index}`
					...
					description:
						typeof prof === "string" ? prof : prof.description || prof.level,
				})
			);
	...
	const onSubmit = async (data) => {
		...
		try {
			...
			if (
				...
				toast.error(
					"The selected SpeakIn name is already taken. Please choose a different one."
				);
			} else {
	...
	const handleRemoveLanguage = (indexToRemove) => {
		...
		if (indexToRemove !== 0) {
			setSelectedLanguages(
				selectedLanguages.filter((_, index) => index !== indexToRemove)
			);
		}
	};
											...
											<div>
												...
												{editMode && (
													<div className="flex items-center justify-between mt-4">
														...
														>
															<FaEdit className="w-4 h-4" /> Edit Teaching
															Language
														</button>
													</div>
												)}
