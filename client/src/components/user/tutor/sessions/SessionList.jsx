...
const SessionsList = ({ sessions, onAddSession, fetchTutorAvailability }) => {
	...
	const isSessionMissed = (session) => {
		...
		const isStudentMissed =
			session.is_booked &&
			session.bookings[0]?.student_joined_within_5_min === false;
		const isTutorMissed =
			session.is_booked &&
			session.bookings[0]?.tutor_joined_within_5_min === false;
	...
	const isSessionListed = (session) => {
		...
		return (
			!session.is_booked && sessionStartTime - currentTime >= threeHoursInMs
		);
	};
	...
	const handleCancelSession = async (session) => {
		...
		const allowedTimeBeforeSessionStart =
			session.session_type === "trial" ? 60 : 120;
	...
	const getRoomNameStatus = (session) => {
		...
		const isConfirmedStatus =
			session.bookings?.[0]?.booking_status === "confirmed" ||
	...
	const renderActionButtons = (session) => {
		...
		return (
			<div className="flex flex-col gap-3">
				...
				{(isUnbooked(session) ||
					getBookingStatus(session) === "confirmed" ||
	...
	const determineCustomStatus = (session) => {
		...
		const threeHoursBeforeStart = new Date(
	...
	const filteredSessions =
		sessionsWithStudentInfo?.filter((session) => {
		...
		<div className="space-y-6">
				...
				<div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
					<div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
						{sessionStatuses.slice(1).map((status) => {
							const count =
								sessionsWithStudentInfo?.filter((session) => {
									if (status.value === "listed")
										return isSessionListed(session);
									if (status.value === "unlisted")
										return !isSessionListed(session);
			...
			>
				...
				<span className="flex-1 text-left font-medium">
					Important Session Rules for Tutors - Please Read
				</span>
				{showRules ? (
							...
							<div className="space-y-3">
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										You can join your session starting{" "}
										<span className="font-medium">5 minutes before</span> the
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										If you don't join within{" "}
										<span className="font-medium">5 minutes after</span> the
										session start time, it will be marked as a "no-show" and you
										won't receive credits for the session.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									<div className="w-1.5 h-1.5 rounded-full bg-blue-600 flex-shrink-0" />
									<p className="text-gray-600 text-sm">
										If both you and the student fail to join within the first 5
										minutes, the session credits will be retained by the
										platform.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										For your no-shows, the student will receive a{" "}
										<span className="font-medium">
											full refund plus 10% bonus credits
										</span>{" "}
										as compensation.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										Trial sessions cannot be canceled within{" "}
										<span className="font-medium">1 hour</span> of the start
										time. Standard sessions cannot be canceled within{" "}
										<span className="font-medium">2 hours</span> of the start
										time.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										If you cancel a session, the student will receive a full
										refund, and frequent cancellations may impact your tutor
										status.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										Unbooked sessions will be automatically unlisted{" "}
										<span className="font-medium">3 hours</span> before the
										start time.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										For standard sessions, you'll receive{" "}
										<span className="font-medium">80%</span> of the session
										credits, with <span className="font-medium">20%</span>{" "}
										retained as a platform fee.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									<div className="w-1.5 h-1.5 rounded-full bg-emerald-600 flex-shrink-0" />
									<p className="text-gray-600 text-sm">
										For trial sessions, you'll receive{" "}
										<span className="font-medium">100%</span> of the session
										credits with no platform fee.
									</p>
								</div>
							</div>
						</div>
			...
			{!filteredSessions || filteredSessions.length === 0 ? (
				...
				<div className="grid gap-6 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin">
					{filteredSessions.map((session) => (
						...
						>
							<div className="flex flex-wrap justify-start items-center mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-100 gap-2">
								...
								{session.bookings &&
									session.bookings[0] &&
									session.bookings[0].booking_status && (
										<span
								...
								<span
									className={`px-3 py-1.5 rounded-full text-sm font-medium ${getStatusStyles(
										getListingStatus(session)
									)}`}
								>
									{getListingStatus(session) === "listed"
										? "Listed"
										: "Unlisted"}
								</span>
							</div>
												...
												<div className="flex-1">
													...
													<div className="mt-2 sm:mt-3">
														...
														<div className="mt-1 sm:mt-2 flex flex-wrap gap-1 sm:gap-2">
															{session.studentInfo?.language_spoken &&
															session.studentInfo.language_spoken.length > 0 ? (
																session.studentInfo.language_spoken.map(
																	(lang, index) => (
																		<span
																			key={index}
											...
											<div className="flex items-center justify-center h-full">
												<p className="text-gray-600 text-center">
													<span className="block text-lg font-medium mb-1">
														{determineCustomStatus(session) ===
														"expired_unbooked" ? (
															<span className="text-yellow-600">
																Session Expired - No Bookings
															</span>
														) : (
															"Unbooked"
														)}
									...
									{isSessionMissed(session) && (
										<div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-4">
											...
											<span className="font-medium text-gray-900">
												{session.credits_required} credits have been retained by
												the platform as per no-show policy
											</span>
										</div>
									)}

									{(getBookingStatus(session) === "completed" ||
										getBookingStatus(session) === "no_show_by_student") && (
										<div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-4">
											<CreditCard className="w-5 h-5 text-gray-500" />
											<span className="font-medium text-gray-900">
												{session.session_type === "standard" ? (
													<>
														This session was valued at{" "}
														{session.credits_required} credits
														<br />
														<span className="text-sm text-gray-600">
															• 80% (
															{Math.floor(session.credits_required * 0.8)}{" "}
															credits) credited to your account • 20% Platform
															fee: {Math.floor(session.credits_required * 0.2)}{" "}
															credits
														</span>
													</>
												) : (
							...
							{(() => {
								...
								if (roomNameStatus) {
									if (roomNameStatus.type === "showRoomName") {
										const cleanedRoomName = cleanRoomName(
											roomNameStatus.roomName
										);
										return (
