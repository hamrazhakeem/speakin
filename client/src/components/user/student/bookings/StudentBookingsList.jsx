...
const StudentBookingsList = ({
	...
	const handleCancelSession = async (session) => {
		...
		const allowedTimeBeforeSessionStart =
			session.availabilityDetails.session_type === "trial" ? 60 : 120;
		...
		if (timeDifferenceInMinutes < allowedTimeBeforeSessionStart) {
			toast.error(
				`Cannot cancel ${
					...
					session.availabilityDetails.session_type === "trial"
						? "1 hour"
						: "2 hours"
				} of start time.`
			);
		...
		try {
			await axiosInstance.patch(
				`tutor-availabilities/${session.availability}/`,
				{
			...
			return (
				<div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-4">
					<span className="font-medium text-gray-900">
						{creditsRequired} Credits have been retained by the platform as per
						no-show policy
					</span>
				</div>
			);
	...
	useEffect(() => {
		const fetchReports = async () => {
			...
			for (const session of completedSessions) {
				try {
					const sessionReports = await studentApi.getBookingReports(
						axiosInstance,
						session.id
					);
		...
		return (
			<div className="text-center py-12">
				...
				<p className="text-gray-600">
					You haven't booked any sessions yet. Browse available tutors to get
					started!
				</p>
			</div>
		);
					...
					{bookingStatuses.slice(1).map((status) => (
						...
						>
							<span className="text-xl sm:text-2xl font-semibold text-gray-900">
								{
									sessions.filter(
										(session) => session.booking_status === status.value
									).length
								}
							</span>
          ...
          rounded-xl hover:bg-amber-100 transition-colors w-full touch-manipulation"
			>
				...
				<span className="flex-1 text-left font-medium">
					Important Session Rules - Please Read
				</span>
				{showRules ? (
							...
							<div className="space-y-3">
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										...
										session start time, the session will be marked as "no-show"
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										If both you and the tutor fail to join within the first 5
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										If the tutor doesn't join the session, you will receive a{" "}
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										If you cancel a session, the refund policy will apply based
										on the cancellation time, and frequent cancellations may
								...
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										...
										<span className="font-medium">20%</span> of the credits you
										pay will be retained as a platform fee, while{" "}
										<span className="font-medium">80%</span> will go to the
										tutor.
									</p>
								</div>
								<div className="flex items-center space-x-2">
									...
									<p className="text-gray-600 text-sm">
										For trial sessions, no platform fee will be charged, and{" "}
										<span className="font-medium">100%</span> of the credits you
										pay will go to the tutor.
									</p>
								</div>
							</div>
						</div>
							...
							<div className="border-b border-gray-100 pb-4 sm:pb-6 mb-4 sm:mb-6">
								<div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
									<div className="flex flex-wrap gap-2">
										<span
											className={`px-3 py-1.5 rounded-xl text-sm font-medium capitalize ${getSessionTypeStyles(
												session.availabilityDetails.session_type
											)}`}
										>
											{session.availabilityDetails.session_type}
										...
										<span
											className={`px-3 py-1.5 rounded-xl text-sm font-medium capitalize ${getStatusStyles(
									...
									<div className="flex items-center text-gray-700">
										...
										<div>
											<p className="font-medium">
												{formatLocalTime(
													session.availabilityDetails.start_time
												)}
											</p>
											<p className="text-sm text-gray-500">
												...
												{getSessionDuration(
													session.availabilityDetails.start_time,
													session.availabilityDetails.end_time
												)}
											</p>
										</div>
									</div>
										...
										<div className="flex items-start space-x-4">
											...
											<div className="flex-1 min-w-0">
												...
												<p className="text-sm text-gray-600 mb-3">
													{session.tutorUserResponse.country ||
												...
												<div>
													...
													<div className="flex flex-wrap gap-2">
														{session.tutorDetails?.language_spoken.map(
															(lang, index) => (
																<span
																	key={index}
										...
										<div className="flex items-center space-x-3">
											...
											<div className="flex-1 min-w-0">
												...
												<p className="mt-1 text-gray-700 truncate">
													{session.availabilityDetails.language_to_teach ||
														"Not specified"}
												</p>
											</div>
										</div>
											...
											{session.booking_status === "completed" &&
												reports[session.id] && (
													<div className="space-y-2">
														<div
															className={`w-full px-4 py-4 rounded-xl font-medium flex items-center justify-center gap-2
            ${
								...
								: "bg-red-50 text-red-600"
														...
														>
															...
															{reports[session.id].status === "responded" && (
																<button
																	onClick={() =>
																		handleViewResponse(reports[session.id])
																	}
														...
														{reports[session.id].status === "pending" && (
															<p className="text-sm text-gray-600 text-center px-4">
																Our team is currently reviewing your report. You
																will be notified once a response is available.
															</p>
														)}
													</div>
												)}
							...
							{(() => {
								...
								if (roomNameStatus) {
									if (
										roomNameStatus.type === "showRoomName" &&
										!isSessionMissed(session)
									) {
											...
											<div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-100">
												<div className="bg-blue-50 rounded-xl p-4 sm:p-6">
													...
													<div className="flex flex-col sm:flex-row gap-3">
														<code
															className="flex-1 bg-white px-4 py-3 rounded-lg text-blue-900 
