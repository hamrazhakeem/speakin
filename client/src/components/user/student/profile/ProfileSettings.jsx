...
const ProfileSettings = () => {
	...
	const handleAddSpokenLanguage = () => {
		...
		const defaultProficiency =
			spokenLanguages.length === 0
				? "Native"
				: spokenProficiencyLevels.find((level) => level.level !== "Native")
						?.level || "";
	...
	const handleAddLanguageToLearn = () => {
		const hasEmptyFields = languagesToLearn.some(
																		...
																		<div className="flex flex-col sm:flex-row gap-4 items-start bg-white p-4 rounded-lg shadow-sm">
																			...
																			{editMode && (
																				<button
																					type="button"
																					onClick={() =>
																						handleRemoveSpokenLanguage(index)
																					}
																		...
																		<div key={index} className="space-y-2">
																			<div className="flex flex-col sm:flex-row gap-4 items-start bg-white p-4 rounded-lg shadow-sm">
																					...
																					>
																						...
																						{platformLanguages.map((lang) => (
																							<option
																								...
																								disabled={languagesToLearn.some(
																									(existing, i) =>
																										i !== index &&
																										existing.language ===
																											(lang.name || lang)
																				...
																				<button
																					type="button"
																					onClick={() =>
																						handleRemoveLanguageToLearn(index)
																					}
																					className="bg-red-100 text-red-600 p-2 rounded-lg hover:bg-red-200 transition-colors mt-2 sm:mt-0"
																					aria-label="Remove language"
																				>
