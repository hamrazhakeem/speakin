						...
						if (!hasBooking) {
							...
							return slot.bookings.some(
								(booking) =>
									...
									(booking.booking_status === "canceled_by_student" &&
										!slot.is_booked)
							);
						}
					});
				...
				{loading ? (
					...
					<div className="overflow-y-auto max-h-[calc(90vh-120px)] pr-2 space-y-3 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                      ...
                      ${
										...
										>
											{slot.session_type.charAt(0).toUpperCase() +
												slot.session_type.slice(1)}{" "}
											Session
										</span>
										<span
											className={`px-3 py-1 rounded-full text-sm font-medium
                      ${
												statusDisplay.color === "text-green-500"
													...
													: statusDisplay.color === "text-blue-500"
													? "bg-blue-50 text-blue-700"
									...
									<div className="space-y-2 mb-4">
										<div className="flex items-center text-gray-600">
											...
											<span>
												{format(toDate(slot.start_time), "MMMM d, yyyy")}
											</span>
										</div>
										<div className="flex items-center text-gray-600">
											<Clock className="h-4 w-4 mr-2" />
											<span>
												{format(toDate(slot.start_time), "h:mm a")} -{" "}
												{format(toDate(slot.end_time), "h:mm a")}
											</span>
										</div>
									</div>
									...
									{slot.is_booked === false && !slot.bookedByYou && (
										<button
											...
											onClick={() =>
												handleBooking(slot.id, slot.credits_required)
											}
											disabled={isBooking}
										>
											{isBooking ? (
