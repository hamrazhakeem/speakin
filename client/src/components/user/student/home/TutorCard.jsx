...
const TutorCard = ({
	...
	return (
		<div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
					...
					{languageToTeach?.map((teachLang, index) => {
						...
						return (
							<div key={index} className="inline-flex items-center">
								...
								<span
									className={`px-3 py-1 rounded-r-full text-sm flex items-center ${verification.bgColor}`}
								>
									{verification.icon}
			...
			{showModal && (
				<AvailabilityModal tutorId={tutor_id} onClose={handleCloseModal} />
			)}
		</div>
	);
};
