...
const TutorBrowse = () => {
	...
	const teachingLanguages = Array.from(
		new Set(
			tutors?.flatMap(
				(tutor) =>
	...
	const spokenLanguages = Array.from(
		new Set(
			tutors?.flatMap(
				(tutor) => tutor?.language_spoken?.map((lang) => lang.language) || []
			)
		)
	).sort();
	...
	const filteredTutors =
		tutors?.filter((tutor) => {
			...
			const matchesTeachLanguage =
				!selectedTeachLanguage ||
				tutor.tutor_language_to_teach?.some(
					(lang) =>
				...
				<div className="container mx-auto px-4">
					<div className="max-w-3xl mx-auto text-center">
						...
						<p className="text-lg sm:text-xl text-blue-100 mb-8">
							Connect with expert language tutors for personalized 1-on-1
							lessons
						</p>
