...
const Navbar = () => {
	...
	const { isAuthenticated, userId, credits, userName, isTutor } = useSelector(
		(state) => state.auth
	);
	...
	useEffect(() => {
		...
		const connectWebSocket = () => {
			if (
				isAuthenticated &&
				userId &&
				(showNotifications || showNotificationsModal)
			) {
				ws = new WebSocket(
				...
				ws.onmessage = async (event) => {
					...
					if (data.type === "send.notification") {
						try {
							const senderResponse = await axiosInstance.get(
							...
							const newNotification = {
								...
								sender_name:
									senderResponse.data.name || `User ${data.sender_id}`,
				...
				response.data.map(async (notification) => {
					try {
						...
						return {
							...notification,
							sender_name:
								senderResponse.data.name || `User ${notification.sender_id}`,
							sender_image: senderResponse.data.profile_image,
						};
					} catch (error) {
						return {
