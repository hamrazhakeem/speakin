...
const formatMessageTime = (timestamp) => {
	...
	return (
		messageDate.toLocaleDateString([], dateOptions) +
		" " +
	...
	useEffect(() => {
		const fetchChatUsers = async () => {
			try {
				...
				const messageResponse = await axiosInstance.get(
		...
		<div className="fixed inset-0 pt-16">
			<div className="flex h-full">
				<div
					className={`${
						isMobile
							? `fixed inset-0 top-16 bg-white z-10 ${
										...
										<Avatar
											...
											name={
												selectedUser.user_type === "student"
													? selectedUser.name
													: selectedUser.tutor_details.speakin_name
											}
											size={40}
											className="rounded-full ring-2 ring-white"
										/>
							...
							>
								{messages.length === 0 ? (
									<div className="flex flex-col items-center justify-center h-full text-gray-500">
										...
										<p className="text-sm text-gray-400">
											Send your first message to tutor{" "}
											{selectedUser.user_type === "student"
												? selectedUser.name
												: selectedUser.tutor_details.speakin_name}
										</p>
