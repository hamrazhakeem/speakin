...
const Card = ({ children, className = "" }) => (
	<div
		className={`bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg transition-shadow duration-200 ${className}`}
	>
		{children}
	</div>
);
	...
	const fetchCreditsHistory = async () => {
		try {
			...
			const response = await axiosInstance.get(
				`bookings/tutor-credits-history/${userId}/`
			);
			...
			if (response.data) {
				...
				toast.success(
					`Withdrawal successful! Amount: ₹${
						response.data.amount_inr
					} (USD ${response.data.amount_usd.toFixed(2)})`
				);
				fetchTransactions();
				checkStripeAccount();
			}
			...
			<main className="container mx-auto px-4 py-8">
				<div className="text-center mb-24 mt-16">
					...
					<p className="text-gray-600 max-w-2xl mx-auto">
						{isTutor
							? "Convert your earned credits to real money through Stripe."
							: "Convert your credits to real money through Stripe."}
					</p>
					...
					<div className="max-w-sm mx-auto mt-6">
						<PrimaryButton
							onClick={() =>
								navigate(isTutor ? "/tutor/dashboard" : "/profile")
							}
						>
									...
									<div className="space-y-4">
										...
										>
											{isLoading
												? "Processing..."
												: `Withdraw ₹${
														credits ? (credits * 150).toLocaleString() : "0"
												  }`}
										</GradientButton>
									</div>
								)}
									...
									{isLoadingCreditsHistory ? (
										...
										<div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto scrollbar-thin rounded-lg">
											{creditsHistory.map((credit) => (
												<CreditsHistoryItem key={credit.id} credit={credit} />
											))}
										</div>
									) : (
										<div className="text-center py-8">
