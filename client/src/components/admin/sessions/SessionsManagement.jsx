...
const SessionsManagement = () => {
	...
	const fetchSessions = async () => {
		try {
			...
			const [tutorRes, studentRes] = await Promise.all([
				Promise.all(
					[...tutorIds].map((id) => adminApi.getUserDetails(axiosInstance, id))
				),
				Promise.all(
					[...studentIds].map((id) =>
						adminApi.getUserDetails(axiosInstance, id)
					)
				),
			]);
	...
	const TabButton = ({ label, isActive, onClick }) => (
		<AdminButton variant="tab" active={isActive} onClick={onClick} size="sm">
			{label}
		</AdminButton>
	);
	...
	const renderEmptyState = () => (
		<div className="col-span-1 md:col-span-2 lg:col-span-3 flex flex-col items-center justify-center min-h-[400px]">
			<p className="text-lg text-zinc-400 mb-2">
				No {activeTab === "availabilities" ? "availabilities" : "bookings"}{" "}
				found
			</p>
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
				<div>
					...
					<p className="text-zinc-400">
						View and manage all sessions and availabilities
					</p>
				</div>
