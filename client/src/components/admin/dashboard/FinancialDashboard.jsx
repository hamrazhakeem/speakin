...
const FinancialDashboard = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				setLoading(true);
				const [transactionsData, bookingsData, escrowData, availabilitiesData] =
					await Promise.all([
	...
	const calculateStats = () => {
		const filteredTransactions = filterByTimeframe(
			transactions,
			"transaction_date"
		);
		...
		const revenue = filteredTransactions
			.filter(
				(t) =>
					t.transaction_type === "credit_purchase" && t.status === "completed"
			)
		...
		const totalRefundCreditsExpenses = filteredBookings.reduce(
			(sum, booking) => sum + calculateRefundCreditsExpense(booking),
			0
		);
		...
		const platformFeeFromSessions = filteredBookings
			.filter((booking) => {
				...
				const isStandardSession =
					availabilities[booking.availability]?.session_type === "standard";
				return isCompleted && isStandardSession;
			})
			.reduce((sum, booking) => {
				const credits =
					availabilities[booking.availability]?.credits_required || 0;
		...
		const retainedCredits = filteredBookings
			.filter((booking) => {
				...
				const startTime = new Date(
					availabilities[booking.availability]?.start_time
				); // Convert the booking's start time to a Date object
	...
	const prepareMonthlyData = () => {
		...
		Object.values(monthlyData).forEach((month) => {
			...
			month.minimumGuaranteedProfit =
				month.profit - month.refundCreditsExpenses;
		});
	...
	const getTransactionTypeData = () => {
		...
		return Object.entries(
			filteredTransactions.reduce((acc, t) => {
				if (
					t.status === "completed" &&
					["withdrawal", "credit_purchase"].includes(t.transaction_type)
				) {
					acc[t.transaction_type] =
						(acc[t.transaction_type] || 0) + parseFloat(t.amount);
				}
				return acc;
			}, {})
		).map(([name, value]) => ({
			...
			{loading ? (
				...
				<>
							...
							<div className="h-64 lg:h-80">
								<ResponsiveContainer width="100%" height="100%">
									<BarChart data={monthlyData}>
										...
										/>
										<Bar
											dataKey="minimumGuaranteedProfit"
											name="Minimum Guaranteed Profit"
											fill="#22c55e"
											radius={[4, 4, 0, 0]}
										/>
										<Bar
											dataKey="refundCreditsExpenses"
											name="Refund Credits Expenses"
											fill="#facc15"
											radius={[4, 4, 0, 0]}
										/>
									</BarChart>
					...
					<div className="flex flex-col sm:flex-row gap-4 px-4 lg:px-6 py-4">
						<AdminButton onClick={exportToPDF} variant="secondary" size="md">
							Download as PDF
						</AdminButton>
						<AdminButton onClick={exportToExcel} variant="secondary" size="md">
							Download Excel
						</AdminButton>
					</div>
