...
const LanguageStatsCard = () => {
		...
		const fetchData = async () => {
			try {
				...
				bookingsRes.forEach((booking) => {
					const availability = availabilitiesRes.find(
						(a) => a.id === booking.availability
					);
					if (availability) {
				...
				setLanguageStats({ teaching: [], learning: [], spoken: [] });
			} finally {
				setLoading(false);
			}
		};
	...
	return (
		<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
			{renderLanguageList(
				languageStats.spoken,
				"Top Languages Spoken",
				"text-blue-500"
			)}
			{renderLanguageList(
				languageStats.learning,
				"Top Languages Being Learned",
				"text-purple-500"
			)}
			{renderLanguageList(
				languageStats.teaching,
				"Top Languages Being Taught",
				"text-green-500"
			)}
		</div>
	);
