export const studentApi = {
	signIn: (axios, credentials) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}sign-in/`, credentials)
			.then((response) => response.data),

	signInWithGoogle: (axios, code) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}google-sign-in/`, {
				code: code,
			})
			.then((response) => response.data),
	...
	createCheckoutSession: (axiosInstance, data) =>
		axiosInstance
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}create-checkout-session/`, {
				user_id: data.userId,
				amount: data.amount,
	...
	getBookingReports: (axiosInstance, bookingId) =>
		axiosInstance.get(`reports/${bookingId}`).then((response) => response.data),
	...
	getReports: (axiosInstance) =>
		axiosInstance.get("reports/").then((response) => response.data),

	getReport: (axiosInstance, reportId) =>
		axiosInstance.get(`reports/${reportId}/`).then((response) => response.data),
};
