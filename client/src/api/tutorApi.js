export const tutorApi = {
	signIn: (axios, data) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}tutor-sign-in/`, {
				email: data.email,
	...
	sendVerificationOtp: (axios, email) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}tutor/verify-email/`, {
				email,
			})
			.then((response) => response.data),
	...
	verifyOtp: (axios, data) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}tutor/verify-otp/`, {
				email: data.email,
				otp: data.otp,
	...
	resendOtp: (axios, data) =>
		axios
			.post(`${import.meta.env.VITE_API_GATEWAY_URL}resend-otp/`, {
				email: data.email,
				cache_key: data.cache_key,
			})
			.then((response) => response.data),
	...
	getCountries: (axiosInstance) =>
		axiosInstance.get("countries/").then((response) => response.data),

	getSpokenLanguages: (axiosInstance) =>
		axiosInstance.get("spoken-languages/").then((response) => response.data),
	...
	getPlatformLanguages: (axiosInstance) =>
		axiosInstance
			.get(`${import.meta.env.VITE_API_GATEWAY_URL}platform-languages/`)
			.then((response) => response.data),
	...
	submitLanguageChangeRequest: (axiosInstance, formData) =>
		axiosInstance
			.post(
				...
				}teaching-language-change-requests/`,
				formData,
				{
					headers: {
	...
	getTeachingLanguage: () => sessionStorage.getItem("teachingLanguage"),
	...
	createSession: (axiosInstance, sessionData) =>
		axiosInstance
			.post("tutor-availabilities/", sessionData)
			.then((response) => response.data),
};
