    ...
    def has_permission(self, request, view):
        ...
        if user_id_from_data:
            if int(user_id_from_data) != user_id:
                logger.warning(
                    f"Permission denied: token user_id {user_id} does not match request user_id {user_id_from_data}"
                )
            ...
            if int(user_id_from_url) != user_id:
                logger.warning(
                    f"Permission denied: token user_id {user_id} does not match URL user_id {user_id_from_url}"
                )
                raise PermissionDenied(
                    "You do not have permission to access this resource."
                )
            ...
            logger.warning(
                "Permission denied: user_id not found in request data or URL"
            )
