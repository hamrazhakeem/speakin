...
class CreateCheckoutSession(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                logger.info(
                    f"Creating checkout session for user {user_id}: {credits} credits"
                )
    ...
    def post(self, request):
        try:
            ...
            event = stripe.Webhook.construct_event(
                ...
                tolerance=360000000000000000000000000000,
            )
...
class StripeAccountDetail(generics.RetrieveAPIView):
    ...
    def get(self, request, user_id):
        try:
            ...
            return Response(
                {"error": "Stripe account not found for the specified user."},
...
class StripeAccountList(generics.ListCreateAPIView):
    ...
    def post(self, request):
        try:
            ...
            if existing_account:
                logger.info(
                    f"Creating account links for existing account: {existing_account.stripe_account_id}"
                )
            ...
            StripeAccount.objects.create(
                user_id=request.data.get("user_id"), stripe_account_id=account.id
            )
...
class WithdrawView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            user_id = request.data.get("user_id")
            logger.info(
                f"Processing withdrawal for user {user_id}: {credits} credits ({amount_inr} INR)"
            )
            ...
            if not stripe_account.is_verified:
                logger.warning(
                    f"Withdrawal rejected: Unverified Stripe account for user {user_id}"
                )
                return Response(
            ...
            logger.info(
                f"Creating transfer of {amount_usd_cents} USD cents to account {stripe_account.stripe_account_id}"
            )
            ...
            if not grpc_success:
                logger.error(
                    f"Failed to update user credits, reversing transfer for user {user_id}"
                )
                stripe.Transfer.create_reversal(transfer.id, amount=amount_usd_cents)
...
class TransactionsDetail(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Transactions.objects.filter(user_id=user_id).order_by(
            "-transaction_date"
        )
