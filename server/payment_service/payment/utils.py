...
def handle_checkout_completed(session):
    try:
        ...
        logger.info(
            f"Processing checkout completion for user {user_id}: {credits} credits"
        )
        ...
        if not grpc_success:
            logger.error(
                f"Failed to update user credits for user {user_id}, initiating refund"
            )
    ...
    try:
        if event_type == "file.created":
            ...
            account_details = stripe.Account.list(
                limit=1, created={"gte": account.get("created", 0) - 60}
            )
            ...
            stripe_account.save()
            logger.info(
                f"Account {account.id} verification status updated to {is_verified}"
            )
