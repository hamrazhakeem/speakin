...
class PaymentService(payment_service_pb2_grpc.PaymentServiceServicer):
    def LockCredits(self, request, context):
        with connection.cursor():
            try:
                logger.info(f"Locking credits for booking {request.booking_id}: student {request.student_id}, tutor {request.tutor_id}")
    ...
    def RefundLockedCredits(self, request, context):
        with connection.cursor():
            try:
                ...
                logger.info(f"Credits refunded successfully for booking {request.booking_id}")
                return payment_service_pb2.RefundLockedCreditsResponse(success=True)
            except Escrow.DoesNotExist:
                logger.warning(f"Escrow transaction not found for booking {request.booking_id}")
                ...
                logger.error(f"Error processing refund for booking {request.booking_id}: {str(e)}")
    ...
    def ReleaseLockedCredits(self, request, context):
        with connection.cursor():
            try:
                logger.info(f"Processing credit release for booking {request.booking_id}")
                escrow_transaction = Escrow.objects.get(
                    booking_id=request.booking_id,
                ...
                if escrow_transaction.status == 'released':
                    logger.info(f"Credits already released for booking {request.booking_id}")
                ...
                if session_type == "standard":
                    ...
                    logger.info(f"Releasing standard session credits ({credits}) to tutor {user_id}")
                    update_user_credits(user_id, credits) 
                    
                elif session_type == "trial":
                    logger.info(f"Releasing trial session credits ({credits_locked}) to tutor {user_id}")
                    update_user_credits(user_id, credits_locked)

                logger.info(f"Credits released successfully for booking {request.booking_id}")
                return payment_service_pb2.RefundLockedCreditsResponse(success=True)
                ...
                logger.error(f"Error releasing credits for booking {request.booking_id}: {str(e)}")
