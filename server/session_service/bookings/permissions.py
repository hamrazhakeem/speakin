...
class IsAdminOrOwnerPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        if isinstance(view, BookingsDetail):
            ...
            )  # Assuming 'pk' is passed in the URL
            if booking.student_id == user_id:
                logger.info(
                    f"Access granted to student {user_id} for booking {booking.id}"
                )
                return True  # Allow student to update their own booking or tutor to update their booking
            else:
                logger.warning(
                    f"Access denied: User {user_id} attempted to modify booking {booking.id}"
                )
        ...
        try:
            ...
            if availability.tutor_id == user_id:
                logger.info(
                    f"Access granted to tutor {user_id} for availability {availability_id}"
                )
                return True

            # Check if the student has a confirmed booking for the given availability
            booking = Bookings.objects.filter(
                availability=availability, student_id=user_id
            ).first()
            if booking:
                logger.info(
                    f"Access granted to student {user_id} for booked availability {availability_id}"
                )
                return True
...
class ValidateRoomNamePermission(BasePermission):
    ...
    def has_permission(self, request, view):
        try:
            ...
            logger.warning(
                f"Invalid booking or unauthorized access attempt by user {user_id}"
            )
