...
class TutorAvailabilityList(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        logger.info(f"Tutor availability created successfully: ID {serializer.instance.id}")
...
class TutorAvailabilityDetail(generics.RetrieveUpdateDestroyAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not success:
                logger.error(f"Failed to refund credits from escrow for booking {booking.id}")
    ...
    def update(self, request, *args, **kwargs):
        ...
        if 'student_joined_at' in request.data:
            if valid_time_window_start <= current_time <= valid_time_window_end:
                ...
                logger.info(f"Student joined within 5-minute window for booking {booking.id}")
        ...
        if 'tutor_joined_at' in request.data:
            if valid_time_window_start <= current_time <= valid_time_window_end:
                ...
                logger.info(f"Tutor joined within 5-minute window for booking {booking.id}")
        ...
        if 'booking_status' in request.data and request.data['booking_status'] == 'completed':
            ...
            if booking.student_joined_within_5_min and booking.tutor_joined_within_5_min:
                if release_credits_from_escrow(session_type, booking_id=booking.id):
                    ...
                    logger.info(f"Session completed successfully for booking {booking.id}")
                else: 
                    logger.error(f"Failed to release credits from escrow for booking {booking.id}")
                ...
                if refund_credits_from_escrow(booking_id=booking.id):
                    if update_user_credits(
                        ...
                        booking.refund_status = True
                        logger.info(f"No-show by tutor processed successfully for booking {booking.id}")
                    else:
                        logger.error(f"Failed to update user credits for booking {booking.id}")
    ...
    def create_meeting_token(self, room_name):
        ...
        data = {
            "properties": {
                "room_name": room_name, 
                "is_owner": False,
                "exp": int((timezone.now() + timezone.timedelta(hours=1.5)).timestamp()),
            }
        }
        response = requests.post(DAILY_API_URL, json=data, headers=headers)
        response.raise_for_status()
...
class ReportList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        logger.info(f"Fetching reports for booking: {booking_id}" if booking_id else "Fetching all reports")
        if booking_id:
