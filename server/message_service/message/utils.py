...
def get_user_by_id(user_id):
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"User service returned error {response.status_code} for user ID {user_id}")
            raise PermissionDenied("User not found in user service or invalid ID")
    except requests.exceptions.RequestException as e:
        logger.error(f"Failed to connect to user service for user ID {user_id}: {str(e)}")
