...
class LanguageSpokenSerializer(serializers.ModelSerializer):
    ...
    proficiency = serializers.SlugRelatedField(
        slug_field="level", queryset=Proficiency.objects.all()
    )
...
class TutorDetailsSerializer(serializers.ModelSerializer):
    language_spoken = LanguageSpokenSerializer(
        many=True, read_only=True, source="user.language_spoken"
    )
...
class TeachingLanguageChangeRequestSerializer(serializers.ModelSerializer):
    ...
    tutor_language_to_teach = TutorLanguageToTeachSerializer(
        many=True, source="user.tutor_language_to_teach", read_only=True
    )
    ...
    def to_internal_value(self, data):
        ...
        if new_language_name:
            try:
                ...
                raise serializers.ValidationError(
                    {"new_language": f"Language '{new_language_name}' not found."}
                )
    ...
    def validate_spoken_languages(self, value):
        ...
        for lang in value:
            if not lang.get("proficiency"):
                raise serializers.ValidationError(
                    "Proficiency level is required for each language."
                )
            if lang["proficiency"] not in valid_proficiencies:
                raise serializers.ValidationError(
                    f"Invalid proficiency level: {lang['proficiency']}"
                )
