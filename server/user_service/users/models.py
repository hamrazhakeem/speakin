...
class User(AbstractUser):
    ...
    profile_image = models.ImageField(
        storage=S3Boto3Storage(), upload_to="profile_images/", blank=True, null=True
    )
...
class TutorDetails(models.Model):
    user = models.OneToOneField(
        User, related_name="tutor_details", on_delete=models.CASCADE
    )
    ...
    intro_video = models.FileField(
        storage=S3Boto3Storage(), upload_to="tutor_intro_videos/"
    )
    certificate = models.ImageField(
        storage=S3Boto3Storage(),
        upload_to="tutor_certificate_images/",
        blank=True,
        null=True,
    )
    govt_id = models.ImageField(
        storage=S3Boto3Storage(),
        upload_to="tutor_govt_id_images/",
        blank=True,
        null=True,
    )
...
class LanguageSpoken(models.Model):
    user = models.ForeignKey(
        User, related_name="language_spoken", on_delete=models.CASCADE
    )
...
class LanguageToLearn(models.Model):
    user = models.ForeignKey(
        User, related_name="language_to_learn", on_delete=models.CASCADE
    )
    ...
    proficiency = models.ForeignKey(
        Proficiency, on_delete=models.CASCADE, limit_choices_to={"level__ne": "Native"}
    )
...
class TutorLanguageToTeach(models.Model):
    user = models.ForeignKey(
        User, related_name="tutor_language_to_teach", on_delete=models.CASCADE
    )
    language = models.ForeignKey(
        Language,
        on_delete=models.CASCADE,
...
class TeachingLanguageChangeRequest(models.Model):
    user = models.ForeignKey(
        User, related_name="language_change_requests", on_delete=models.CASCADE
    )
    profile_image = models.ImageField(
        storage=S3Boto3Storage(), upload_to="profile_images/", blank=True, null=True
    )
