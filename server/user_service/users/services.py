...
class EmailService:
    ...
    def send_registration_email(email, user_data):
        ...
        send_email_task.delay(
            "Your OTP for SpeakIn Registration", email, template, context
        )
    ...
    def resend_registration_otp(email, cache_key):
        ...
        send_email_task.delay(
            "Resend: Your OTP for SpeakIn Registration", email, template, context
        )
    ...
    def send_forgot_password_email(email):
        ...
        send_email_task.delay(
            "Your OTP for SpeakIn Password Reset", email, template, context
        )
    ...
    def resend_forgot_password_otp(email, cache_key):
        ...
        send_email_task.delay(
            "Resend: Your OTP for Password Reset", email, template, context
        )
    ...
    def send_tutor_approval_email(user):
        ...
        context = format_email_context(
            {"tutor_name": user.name, "signin_url": signin_url}
        )
        template = get_email_template_path("tutor_approval_email")

        send_email_task.delay(
            "Your Tutor Account has been Approved!", user.email, template, context
        )

    @staticmethod
    def send_tutor_denial_email(user):
        """Send tutor denial email"""
        context = format_email_context({"tutor_name": user.name})
    ...
    def send_language_change_approval_email(user, language_name, tutor_name):
        ...
        context = format_email_context(
            {"language": language_name, "tutor_name": tutor_name}
        )
        template = get_email_template_path("language_to_teach_change_approval_email")

        send_email_task.delay(
            "SpeakIn Language Change Approval", user.email, template, context
        )
    ...
    def send_language_change_denial_email(user, new_language, tutor_name, old_language):
        ...
        send_email_task.delay(
            "SpeakIn Language Change Denial", user.email, template, context
        )
    ...
    def send_account_suspension_email(user):
        ...
        context = format_email_context({"user_name": user.name})
