...
class CreateCheckoutSessionView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.info("Forwarding checkout session creation request to payment service")
...
class StripeAccountView(APIView):
    ...
    def get(self, request, user_id):
        try:
            ...
            logger.info(f"Forwarding Stripe account fetch request to payment service for user {user_id}")
            ...
            logger.error(f"Failed to forward Stripe account fetch request to payment service: {str(e)}")
            return Response(
                {"error": "Failed to connect to payment service.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ) 
    ...
    def post(self, request):
        try:
            ...
            logger.error(f"Failed to forward Stripe account creation request to payment service: {str(e)}")
            return Response(
                {"error": "Failed to connect to payment service.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )  
...
class WithdrawView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.error(f"Failed to forward withdrawal request to payment service: {str(e)}")
            return Response(
                {"error": "Failed to connect to payment service.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def get(self, request, user_id=None):
        try:
            if user_id:
                ...
                logger.info(f"Forwarding transactions fetch request to payment service for user {user_id}")
            else:
                payment_service_url = os.getenv('PAYMENT_SERVICE_URL') + 'transactions/'
                logger.info("Forwarding transactions fetch request to payment service for all users")
            ...
            logger.error(f"Failed to forward transactions fetch request to payment service: {str(e)}")
            return Response(
                {"error": "Failed to connect to payment service.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class EscrowView(APIView):
    ...
    def get(self, request):
        try:
            ...
            logger.error(f"Failed to forward escrow details fetch request to payment service: {str(e)}")
            return Response(
                {"error": "Failed to connect to payment service.", "details": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )