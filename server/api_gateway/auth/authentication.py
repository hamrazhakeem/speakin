...
class JWTAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            logger.warning(f"Authentication failed: Token missing in request from {request.META.get('REMOTE_ADDR')}")
        ...
        try:
            ...
            logger.warning(f"Authentication failed: Token expired for request from {request.META.get('REMOTE_ADDR')}")
